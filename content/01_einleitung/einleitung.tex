...
Entwickler und Betreiber von JavaScript-basierten Webapplikationen werden fast alltäglich damit konfrontiert, dass sie beispielsweise zu Zwecken der Problembehebung nachvollziehen müssen, wie ein Nutzer die Applikation verwendet und wie sich die Applikation dabei verhält. Dies ist in der Umgebung von Browsern deutlich schwieriger, als in anderen Projekten mit Nutzerinteraktion. Denn es gibt von Haus aus keine Möglichkeit über Logs das Verhalten nachzuvollziehen, weil die Logs nur beim Client einsehbar sind. Weiterhin muss man bei einer eigenen Lösung zur Informationsgewinnung mit der JavaScript-Sandbox im Browser umgehen, und CORS-Vorgaben einhalten.

Mit diesen Schwierigkeiten bei Webapplikationen wird sich in dieser Arbeit beschäftigt, um nachhaltig eine Nachvollziehbarkeit zu erreichen.
