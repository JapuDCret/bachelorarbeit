...
%\subsection{Nutzen}
%
%Tritt beispielsweise ein Softwarefehler (Bug) bei einem Nutzer auf, aber die Betreiber erhalten nicht ausreichende Informationen, so kann der Bug ignoriert oder gering priorisiert werden und in Vergessenheit geraten. Dies geschah im Jahr 2013, als Khalil Shreateh eine Sicherheitslücke bei Facebook fand und diese bei Facebooks Bug-Bounty-Projekt meldete \cite{FacebookBugBountyHunt}. Sein Fehlerreport wurde aufgrund mangelnder Daten abgelehnt:
%
%\begin{quotation}
%\enquote{Unfortunately your report [...] did not have enough technical information for us to take action  on  it. We  cannot  respond  to  reports  which  do  not contain enough detail to allow us to reproduce an issue.}
%\end{quotation}
%
...
\footnotetext{Eine Anfrage ist \enquote{simple}, wenn 1. der Methode GET, HEAD oder POST entspricht; 2. sie keine eigene Header enthält; und 3. der \enquote{Content-Type} von POST-Anfragen einem der folgenden Werte entspricht: \enquote{application/x-www-form-urlencoded}, \enquote{multipart/form-data} oder \enquote{text/plain} \cite{MDNCORS}.}
